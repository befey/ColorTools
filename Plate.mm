...
void Plate::UpdateDate() {
	...
	if (hDate) {
		...
		dateRange.Remove();
        ASUnicode* asuString = new ASUnicode [80];
        StdStringToASUnicode(buffer, asuString, 80);
		dateRange.InsertBefore(asuString);
	}
			...
			while ( currTextRunsIterator.IsNotDone() ) {
				...
				if (newBounds.right > MAX_WIDTH_POINT) {
					...
					if (newBounds.left == startBounds.left) {
						...
						//Insert a return
                        ASUnicode* asuString = new ASUnicode [3];
                        StdStringToASUnicode("\r", asuString, 3);
						currTextRunsIterator.Item().InsertBefore(asuString);
